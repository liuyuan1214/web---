...
<template>
   <div style="padding-bottom:70px;min-height:470px;">
     ...
     <div class="header f-df f-pr20">
       <div>
           <label></label>
          <el-input placeholder="请输入产品名称" style="vertical-align:bottom"  @keyup.enter.native=getTableData v-model.trim=findName class="input-with-select"  size="small">
          </el-input> 
    ...
    export default {
        ...
        data(){
            return {
                ...
                //table参数
                tableHeader:[
                  {label:'场所名称1',prop:'cdmc'},
                  {label:'养殖产品',prop:'ztncpmc'},
                  {label:'养殖批次',prop:'zzpc'},
                  {label:'存栏数',prop:'cls'},
                  {label:'操作',width:'120','render-column':(h,row)=>{
                     return (
                           <el-button size="mini" class="edit"  onClick={()=>{this.$emit('selPc',row)}}>选择</el-button>
                        )
                  }}
                ],
                loading:false,
