  ...
  <div class="main-info">
    ...
    <div class="baseinfo">
      <div class="f-df row">
        ...
        <div class="f-f1">
          <el-input :class="reqiredKeys.ztjc?'border-red':''" placeholder="请输入" :maxlength="16" type="text" v-if="edit"
                    v-model.trim="infos.ztjc"/>
      ...
      <div class="f-df row">
        ...
        <div class="f-f1">
          <el-input :class="reqiredKeys.frdb?'border-red':''" placeholder="请输入" :maxlength="20" show-word-limit
                    type="text" v-if="edit" v-model.trim="infos.frdb"/>
      ...
      <div class="f-df row">
        ...
        <div class="f-f1">
          <el-input :class="reqiredKeys.lxr?'border-red':''" placeholder="请输入" :maxlength="20" show-word-limit
                    v-if="edit" v-model.trim="infos.lxr"/>
          <span v-else>{{infos.lxr}}</span>
        </div>
        <div class="title"><span class="must-icon">*</span>联系电话</div>
        <div class="f-f1">
          <el-input :class="reqiredKeys.lxdh?'border-red':''" placeholder="请输入" :maxlength="20" show-word-limit
                    type="number" v-positiveNumber v-if="edit" v-model.trim="infos.lxdh"/>
      ...
      <div class="f-df row">
        ...
        <div class="f-f1">
          <el-input :class="reqiredKeys.dz?'border-red':''" placeholder="请输入" :maxlength="60" show-word-limit
                    type="text" v-if="edit" v-model.trim="infos.dz"/>
        ...
        <div class="f-f1 f-df f-pr">
          <template v-if="edit">
            ...
            <el-input><span slot="prefix">x：{{infos.xzb}}<span class="f-pr20 f-pl20"></span>y：{{infos.yzb}}</span><span
              slot="suffix" class="f-fs24 f-pr10">···</span></el-input>
      ...
      <div class="f-df row">
        ...
        <div class="f-f1">
          <el-input placeholder="请输入" :maxlength="20" v-if="edit" type="number" v-positiveNumber
                    v-model.trim="infos.positCode"/>
          <span v-else>{{infos.positCode}}</span>
        </div>
        <div class="title">从业人数</div>
        <div class="f-f1">
          <el-input placeholder="请输入" :maxlength="20" type="number" v-positiveNumber v-if="edit"
                    v-model.trim="infos.cyrs"/>
      ...
      <div class="f-df row">
        ...
        <div class="f-f1">
          <el-input :class="reqiredKeys.zdmj?'border-red':''" placeholder="请输入" :maxlength="10" show-word-limit
                    type="number" v-positiveNumber v-if="edit" v-model.trim="infos.zdmj"/>
          <span v-else>{{infos.zdmj}}</span>
        </div>
        <div class="title"><span class="must-icon">*</span>种养殖面积(亩)</div>
        <div class="f-f1">
          <el-input :class="reqiredKeys.scmj?'border-red':''" placeholder="请输入" :maxlength="10" show-word-limit
                    type="number" v-positiveNumber v-if="edit" v-model.trim="infos.scmj"/>
      ...
      <div class="f-df row">
        ...
        <div class="f-f1">
          <el-select :class="reqiredKeys.nyzt?'border-red':''" v-model.trim=infos.nyzt v-if="edit" style="width:100%;">
            <el-option value="1" label="有限公司"></el-option>
            <el-option value="2" label="专业合作社"></el-option>
            <el-option value="3" label="家庭农场"></el-option>
            <el-option value="4" label="其他"></el-option>
          </el-select>
          <span v-else>{{infos.nyzt==1? '有限公司':infos.nyzt==2 ?'专业合作社':infos.nyzt==3 ?'家庭农场':'其他'}}</span>
        </div>
        <div class="title"><span class="must-icon">*</span>企业级别</div>
        <div class="f-f1">
          <el-select :class="reqiredKeys.scqylx?'border-red':''" v-model.trim=infos.scqylx v-if="edit"
                     style="width:100%;">
            <el-option value="1" label="农业生产企业"></el-option>
            <el-option value="2" label="加工企业"></el-option>
            <el-option value="3" label="非食用产品企业"></el-option>
          </el-select>
          <span v-else>{{infos.scqylx==1? '农业生产企业':infos.scqylx==2 ?'加工企业':'非食用产品企业'}}</span>
        </div>
      </div>
      <div class="f-df row">
        <div class="title"><span class="must-icon">*</span>是否龙头企业</div>
        <div class="f-f1" style="text-indent: 5px;" :style="isltqy?'border:solid 2px red':''">
          <el-radio v-model="infos.ltqy" :label="1" @change="selltqy(1)" :disabled="!edit">是</el-radio>
          <el-radio v-model="infos.ltqy" :label="2" @change="selltqy(2)" :disabled="!edit">否</el-radio>
        </div>

        <div class="title" v-show="infos.ltqy == 1"><span class="must-icon">*</span>主体级别</div>
        <div class="f-f1" v-show="infos.ltqy == 1">
          <el-select :class="infos.ltqy == 1&&infos.company == ''||infos.company == undefined||infos.company == null?'border-red':''" v-model.trim=infos.company v-if="edit"
                     style="width:100%;">
      ...
      <div class="f-df row f-pr">
        ...
        <div class="f-f1" style="margin-left:200px">
          <el-input placeholder="请输入，500字以内" :maxlength="500" show-word-limit type="textarea"
                    :autosize="{ minRows: 3, maxRows: 10}" v-if="edit" v-model.trim="infos.qyjs"/>
    ...
    <div class="baseinfo">
      ...
      <div class="f-df row">
        ...
        <div class="f-f1">
          <el-input placeholder="请输入" :maxlength="30" show-word-limit type="text" v-if="edit"
                    v-model.trim="infos.zczb"/>
          <span v-else>{{infos.zczb}}</span>
        </div>
        <div class="title">登记机关</div>
        <div class="f-f1">
          <el-input placeholder="请输入" :maxlength="30" show-word-limit type="text" v-if="edit"
                    v-model.trim="infos.djjg"/>
      ...
      <div class="f-df row f-pr">
        ...
        <div class="f-f1" style="margin-left:200px;">
          <el-input placeholder="请输入" :maxlength="500" show-word-limit type="textarea"
                    :autosize="{ minRows: 3, maxRows: 10}" v-if="edit" v-model.trim="infos.jyfw"/>
    ...
    methods: {
      ...
      getZtInfo() {
        ...
        this.$fetch.api_user.user_comInfo(params).then((res) => {
          ...
          let {tyshxydm, jyzt, scjyqyId, positCode, gsyx, gswz, zczb, yyqx, clrq, fzrq, djjg, jyfw,scqylx,nyltqy} = res.beanTemp;
          this.infos = {
            qydm,
            company,
            nyzt,
      ...
      submit() {
        if (this.infos.ltqy == undefined) {
          this.isltqy = true;
          this.$message.error('请按要求填写信息！');
          return
        }
        let {nyzt, qydm, company, xzb, yzb, qyjs, ztdm, ztmc, frdb, scpz, lxr, lxdh, dz, cyrs, zdmj, scmj, ztlb, ztjc, tyshxydm, jyzt, scjyqyId, positCode, gsyx, gswz, zczb, yyqx, clrq, fzrq, djjg, jyfw, ltqy,scqylx} = this.infos;
        if (ltqy == 2) {
          company = ''
        }else{
