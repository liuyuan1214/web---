/*关联批次*/
<template>
  <div style="padding-bottom:70px;min-height:500px;">
     <!-- 搜索头部 -->
    ...
    export default {
        ...
        data(){
            return {
                ...
                //table参数
                tableHeader:[
                  {label:'起批/包装日期',prop:'qprq',width:'120'},
                  {label:'样品名称',prop:'ztncpmc'},
                  {label:'地块',prop:'cdmc'},
                  {label:'批次号',prop:'zzpc',width:'230'},
                  {label:'操作',width:'120','render-column':(h,row)=>{
                     return (
                           <el-button size="mini" class="edit"  onClick={()=>{this.selPc(row)}}>选择</el-button>
                        )
                  }}
                ],
                //table参数
                tableHeader1:[
                  {label:'采购/包装日期',prop:'purchDate',width:'120'},
        ...
        */
        getTableData(obj={pageSize:10,pageNo:1}){
          ...
          var query = {
            method:'get',
            url:this.sampleType==1?'/scgl/ns/pageZzjlAndTabNcpPackRecord/':'/cggl/cg/pagePurchDetailAndPack/',
            data:params
          }
         ...
         selPc(row){
           let params = {
             id:this.selId,
             zzpc:this.sampleType==1?row.zzpc:row.batchno,
             scjyztId:this.userInfo.scjyztId,
             type:this.sampleType
           };
           fetch({
             url:'/lcjc/glpcLcjxZcb/',
             method:'post',
             data:params
           }).then(res=>{
