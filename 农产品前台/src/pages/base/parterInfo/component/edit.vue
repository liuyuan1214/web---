...
<template>
    <el-form ref="form" :model="form" label-width="100px" class="form" :rules="rules" status-icon>
      <div class="form-title f-fs18  f-tac f-mb30 f-mt30">{{form.khId?'编辑':'新增'}}供应商信息</div>
      <el-form-item label="单位类型" prop="lb" >
        <el-select v-model.trim="form.lb"  placeholder="请选择"  style="width:100%" :disabled="isSup?true:false"> 
            <el-option
      ...
      submitForm() {
        this.$refs.form.validate((valid) => {
          if (valid) {
            ...
            this.disabled=true;
            //新增字段 
            let { lb,khmc,khjc,lxr,lxdh, bz,dz,sfky} = this.form;
            //用户参数
            let {qydm,qyid,ztdm,scjyztId} = this.userInfo;
            //系统时间
            let params = {lb,khmc,khjc,lxr,lxdh, bz,dz,sfky,qydm,qyid,ztdm,scjyztId}
            if(this.form.khId){
