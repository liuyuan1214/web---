...
<template>
   <div style="padding-bottom:70px;min-height:470px;">
     ...
     <div class="header f-df f-pr20">
       <div>
           <label>类别</label>
          <el-select v-model.trim="zldm" size="small" class="f-vab f-mr20" style="width:150px;" placeholder="请选择大类"   @change="search();getTypes('NCPEJFL')">
            <el-option
              v-for="item in zldms"
              :key="item.sjzdBm"
              :label="item.sjzdMc"
           ...
           <label>农资类别</label>
          <el-select v-model.trim="ej" size="small"   class="f-vab f-mr20" style="width:150px;" placeholder="请选择小类"  @change="search()">
            <el-option
              v-for="item in ejs"
              :key="item.sjzdBm"
              :label="item.sjzdMc"
      ...
      <!-- <div class="f-f1 f-tar">
        <el-button type="primary" size="medium" @click="isAddFzr=true" round style="padding:9px 25px">申 请</el-button>
      </div> -->
    </div> 
     <s-table
        v-loading=loading
