  ...
  <el-form ref="form" :model="form" label-width="100px" class="form cpwh-edit" :rules="rules" status-icon>
    <div class="form-title f-fs18  f-tac f-mb30 f-mt30">新增投入品申请信息</div>
      ...
      <el-form-item label="农资分类" prop="fenlei" class="f-pr f4">
        <el-input v-model.trim="form.fenlei" placeholder="请选择农资分类" >
...
export default {
  ...
  data() {
    ...
    const commonValidate1 = { required: true, message: '必填项', trigger: ['change'] }
    return {
      disabled: false,
      oldncpmc: '',
      zldms:[],
    ...
    submitForm() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          //用户参数
          let { bizId, bizName, areaCode, userId,ztmc,scjyztId} = this.userInfo;
          //系统时间
